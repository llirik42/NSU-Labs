...
class DNSResolver(SocketLike):
    ...
    def send_request(self, domain_name: str, message_id: int) -> None:
        message_bytes: bytes = self.__generate_dns_request(domain_name=domain_name, message_id=message_id)
        self.__dns_server_socket.send(message_bytes)

    def recv_response(self) -> DNSMessage:
    ...
    @staticmethod
    def __generate_dns_request(domain_name: str, message_id: int) -> bytes:

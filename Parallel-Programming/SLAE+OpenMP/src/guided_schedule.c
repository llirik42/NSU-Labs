...
unsigned int iterate(const struct InputData* input_data, double* result) {
        ...
        #pragma omp parallel default(none) shared(A, b, x0, epsilon, n, max_iterations_count, iterations_count, z, r, A_z, alpha_numerator, alpha, beta, Az_z_ddot, b_nrm_epsilon_squared, r_nrm_squared, checks_count, result)
    {
        ...
        #pragma omp for private(i) schedule(guided, SCHEDULE_PARAMETER) reduction(+:b_nrm_epsilon_squared)
        for (i = 0; i < n; i++) {
        ...
        #pragma omp for private(i, j) schedule(guided, SCHEDULE_PARAMETER) reduction(+:r_nrm_squared)
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
                r[i] += A[i * n + j] * x0[j];
            }
        ...
        #pragma omp single
            ...
            // Calculating A x z_n
            #pragma omp for private(i, j) schedule(guided, SCHEDULE_PARAMETER)
            for (i = 0; i < n; i++) {
                A_z[i] = 0;
...
#pragma omp single
            ...
            #pragma omp for private(i) schedule(guided, SCHEDULE_PARAMETER) reduction(+:Az_z_ddot)
            for (i = 0; i < n; i++) {
                Az_z_ddot += A_z[i] * z[i];
            }
            #pragma omp single
            ...
            #pragma omp for private(i) schedule(guided, SCHEDULE_PARAMETER)
            for (i = 0; i < n; i++) {
                result[i] += z[i] * alpha;
                r[i] = r[i] -  A_z[i] * alpha;
            }
            ...
            #pragma omp for private(i) schedule(guided, SCHEDULE_PARAMETER) reduction(+:r_nrm_squared)
            for (i = 0; i < n; i++) {
                r_nrm_squared += r[i] * r[i];
            }
            #pragma omp single
